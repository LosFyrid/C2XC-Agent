...
function previewText(m: MemoryItem): string {
  // Prefer explicit notes generated by RB (keeps the list usable when `content`
  // is a structured/large payload like RBMEM_CLAIMS_V1).
  const notes = isRecord(m.extra) ? m.extra['notes'] : null
  if (typeof notes === 'string' && notes.trim()) return snippet(notes, 220)

  const s = (m.content ?? '').trim()
  if (s.startsWith('RBMEM_CLAIMS_V1')) return 'RBMEM_CLAIMS_V1 · open to view claims'

  return snippet(m.content, 220) || '—'
}
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 text-xs text-muted">{previewText(m)}</div>
